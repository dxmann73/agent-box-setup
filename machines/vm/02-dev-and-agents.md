...
TypeScript, pnpm, Markdownlint, WezTerm, Firecrawl CLI, Playwright Chromium, QEMU/SPICE guest
agents, and these CLIs:
...
Install SDKMAN, Java, Quarkus, imaging utilities, VS Code, or other optional tools only when the
guest's projects require them. They belong in the `full` verification profile once enabled.
