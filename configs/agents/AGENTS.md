...
- For any web search/research task, first check whether Firecrawl can be used and use it when available.
- Before adding any new dependency, run a GitHub health check and report the results to the user. The report must
  include number of contributors, recent contribution activity, commit frequency, open issues, latest release date, and
  license status.
...
- Destructive operations and commands are forbidden unless explicitly allowed e.g. `reset --hard`, `clean`, `restore`,
  `rm`
